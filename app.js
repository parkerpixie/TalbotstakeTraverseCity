...
{name:'Blake',icon:'🍔',note:'Burgers, low-stress meals, scenery, easy parking.'},
{name:'Porter',icon:'🎮',note:'Clear plans, familiar foods, sweets, unusual stops.'},
...
const shops=[
{id:'m22-tc',name:'M22 Traverse City',town:'Traverse City',type:'gifts',icon:'🌊',url:'https://m22.com/',summary:'Northern Michigan apparel and gifts tied to the road, lakes, and local conservation.'},
{id:'wilson',name:'Wilson Antiques',town:'Traverse City',type:'antiques',icon:'🕰️',url:'https://wilsonantiques.com/',summary:'A large downtown antique destination for vintage treasure hunting and rainy-day wandering.'},
{id:'horizon',name:'Horizon Books',town:'Traverse City',type:'books',icon:'📚',url:'https://www.horizonbooks.com/',summary:'A long-loved independent bookstore downtown with books, gifts, and plenty of browsing room.'},
{id:'cherry-republic-tc',name:'Cherry Republic',town:'Traverse City',type:'food',icon:'🍒',url:'https://www.cherryrepublic.com/',summary:'Cherry salsa, candy, preserves, drinks, gifts, and edible souvenirs with maximum Michigan energy.'},
{id:'m22-sb',name:'M22 Suttons Bay',town:'Suttons Bay',type:'gifts',icon:'🧢',url:'https://m22.com/',summary:'A natural stop while walking St. Joseph Street for apparel and Up North gifts.'},
{id:'nykamping',name:'Nykamping',town:'Suttons Bay',type:'boutique',icon:'🧵',url:'https://nykamping.com/',summary:'Handmade apparel and thoughtfully selected pre-loved textiles with a distinctive local point of view.'},
{id:'m22-leland',name:'M22 Leland',town:'Leland',type:'gifts',icon:'⚓',url:'https://m22.com/',summary:'Easy to pair with Fishtown and the rest of Leland’s compact downtown shopping.'},
{id:'benjamin-maier',name:'Benjamin Maier Ceramics',town:'Leland',type:'art',icon:'🏺',url:'https://benjaminmaierceramics.com/',summary:'Elevated handmade ceramics that feel like a true piece of northern Michigan rather than a generic souvenir.'},
{id:'m22-glen',name:'M22 Glen Arbor',town:'Glen Arbor',type:'gifts',icon:'🏕️',url:'https://m22.com/',summary:'The flagship-feeling Glen Arbor stop for road-trip apparel and outdoor-minded gifts.'},
{id:'cottage-book',name:'The Cottage Book Shop',town:'Glen Arbor',type:'books',icon:'📖',url:'https://www.cottagebooks.com/',summary:'A cozy independent bookstore that pairs perfectly with coffee, lunch, and a Sleeping Bear day.'},
{id:'cherry-republic-ga',name:'Cherry Republic Glen Arbor',town:'Glen Arbor',type:'food',icon:'🍒',url:'https://www.cherryrepublic.com/',summary:'The full cherry wonderland: food, gifts, samples, and an easy family stop.'},
{id:'fishtown',name:'Fishtown Shops',town:'Leland',type:'art',icon:'🎣',url:'https://www.fishtownmi.org/',summary:'A cluster of shanty shops, local goods, art, food, and waterfront atmosphere in one walkable stop.'}
];
...
const allPlaces=[...restaurants.map(x=>({...x,kind:'restaurant'})),...shops.map(x=>({...x,kind:'shop',price:'',tags:[x.type],fit:[]}))];
...
function chooseTraveler(name){selectedTraveler=name;localStorage.setItem('tcTraveler',name);renderTravelers();renderRestaurants();if(dialog.open)dialog.close();showToast(name==='Everyone'?'Showing choices for everyone.':`Showing stronger matches for ${name}.`);}
...
function renderShops(){const q=document.getElementById('shopSearch').value.toLowerCase().trim();const town=document.getElementById('shopTown').value;const type=document.getElementById('shopType').value;const results=shops.filter(s=>town==='all'||s.town===town).filter(s=>type==='all'||s.type===type).filter(s=>`${s.name} ${s.town} ${s.type} ${s.summary}`.toLowerCase().includes(q));document.getElementById('shopGrid').innerHTML=results.length?results.map(placeCard).join(''):'<p>No matches yet. Try another town or treasure type.</p>';attachSaveButtons();}
...
function renderHouseGalleries(){
  ...
  root.innerHTML=houseGalleryData.map((g,index)=>`<section class="gallery-section" data-gallery="${g.id}" data-index="0"><div class="gallery-head"><div><p class="eyebrow dark">${g.kicker}</p><h3>${g.title}</h3><p>${g.description}</p></div><span class="gallery-count">1 / ${g.images.length}</span></div><div class="gallery-stage"><img src="Assets/Assets/Images/${g.images[0]}" alt="${g.title} photo 1" data-open-photo><button class="gallery-control prev" aria-label="Previous photo">‹</button><button class="gallery-control next" aria-label="Next photo">›</button><div class="gallery-caption"><strong>${g.title}</strong><small>Swipe or use arrows</small></div></div><div class="gallery-thumbs">${g.images.map((img,i)=>`<button class="gallery-thumb ${i===0?'active':''}" data-thumb="${i}" aria-label="View ${g.title} photo ${i+1}"><img src="Assets/Assets/Images/${img}" alt=""></button>`).join('')}</div></section>`).join('');
